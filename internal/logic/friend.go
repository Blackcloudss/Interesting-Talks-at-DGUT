...
func NewFriendlogic() *Friendlogic {
	return &Friendlogic{}
}
...
//	@return resp
//	@return err
func (l *Friendlogic) GetFriendList(ctx context.Context, UserId int64) (resp types.GetFriendListResp, err error) {
	defer utils.RecordTime(time.Now())()
	// 获取好友们id
	FriendsId, err := repo.NewFriendRepo(global.DB).GetFriendsId(UserId)
	if err != nil {
		zlog.CtxErrorf(ctx, "获取好友们id失败: %v", err)
		return resp, response.ErrResp(err, GET_FRIENDS_ID)
	}
	// 获取好友信息
	resp.Friends, err = repo.NewFriendRepo(global.DB).GetFriendList(FriendsId)
	if err != nil {
